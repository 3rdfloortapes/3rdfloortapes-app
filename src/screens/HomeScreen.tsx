...
import { fetchHomepageCollections } from '../api/shopify';
...
export default function HomeScreen({ navigation }: any) {
  ...
  useEffect(() => {
    fetchHomepageCollections().then(setCollections).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        ...
        <Text style={styles.tagline}>"A record store without tapes is like a 7-11 without cigarettes." — A. Einstein</Text>
      </View>
